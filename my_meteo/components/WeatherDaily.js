...
export default class WelcomeScreen extends Component {
    ...
    getTemperature = (data) => {
        ...
        data.forEach(element => {
            minTempData = element.main.temp_min < minTempData ? element.main.temp_min : minTempData;
            maxTempData = element.main.temp_max > maxTempData ? element.main.temp_min : maxTempData;
        });
    ...
    formatDate = (inputDate) => {
        ...
        formatTodayDate = today.getDate() + ' / ' + today.getMonth() + ' / ' + today.getFullYear();
        formatTomorrowData = today.getDate() + 1 + ' / ' + today.getMonth() + ' / ' + today.getFullYear();
        ...
        return (
            <View style={{flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
                ...
                <View style={{flex: 1, justifyContent: 'space-around'}}>
                    <Text style={{margin: 5, color:'rgb(0, 160, 217)', textAlign:'right'}}>
                    {this.state.minTemp + '°C '}
                    </Text>
                    <Text style={{margin: 5, color:'red', textAlign:'right'}}>
